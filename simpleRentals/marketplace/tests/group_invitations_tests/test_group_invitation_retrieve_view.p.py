...
class TestGroupInvitationRetrieveView(APITestCase):
    def setUp(self):
        ...
        self.url = reverse('group-invitation-detail', kwargs={'pk': self.invitation.id})
        self.client.force_authenticate(user=self.user)

    def test_retrieve_invitation_success(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['id'], self.invitation.id)

    def test_retrieve_invitation_unauthenticated(self):
        self.client.force_authenticate(user=None)
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_retrieve_invitation_invalid_id(self):
        self.client.force_authenticate(user=self.user)
        invalid_url = reverse('group_invitation_detail', kwargs={'pk': 999})
